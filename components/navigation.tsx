    ...
    <>
      ...
      <nav className="hidden md:flex border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl w-full mx-auto px-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 py-4 font-bold text-lg hover-scale transition-smooth group">
            <BookOpen className="w-6 h-6 text-primary group-hover:rotate-12 transition-transform duration-300" />
            <span className="bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">LibraryHub</span>
            {user && (
              <span
                className={`text-xs px-2 py-1 rounded-full font-medium ${user.role === "admin" ? "bg-primary/10 text-primary border border-primary/30" : "bg-accent/10 text-accent border border-accent/30"}`}
              >
          ...
          <div className="flex items-center gap-2">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href}>
                <Button
                  ...
                  size="sm"
                  className={`gap-2 ${isActive(item.href) ? "hover-glow shadow-md" : "hover-lift"} transition-smooth`}
                >
            ...
            <Button variant="ghost" size="sm" className="gap-2 hover-lift transition-smooth" onClick={toggleTheme}>
              {theme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </Button>

            <Button variant="ghost" size="sm" className="gap-2 hover-lift transition-smooth" onClick={handleLogout}>
      ...
      <nav className="md:hidden border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div className="px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={toggleTheme} className="hover-lift transition-smooth">
              {theme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </Button>
            <Button
              variant="ghost"
