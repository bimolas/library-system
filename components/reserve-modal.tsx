...
export default function ReserveModal({ book, isOpen, onClose, userScore, userLevel }: ReserveModalProps) {
  ...
  const existingReservations = reservations.filter((r) => r.bookId === book.id && r.status !== "cancelled")
  const estimatedPosition = existingReservations.length + 1

  const generateAvailableDates = () => {
    const dates: Date[] = []
    const startDate = book.nextAvailableDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
  ...
  const handleConfirmReserve = async () => {
    ...
    const result = await reserveBook(book.id, user.id, selectedDate, borrowDuration)
        ...
        {step === "success" ? (
          <div className="p-8 text-center">
            ...
            <div className="p-4 bg-primary/5 rounded-lg border border-primary/20">
              <p className="text-sm text-muted-foreground">Expected pickup date:</p>
          ...
          <div className="p-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-accent/10 rounded-lg border border-accent/20">
                  ...
                  <p className="font-bold text-2xl text-accent">#{estimatedPosition}</p>
                </div>
              ...
              <div className="p-3 bg-muted/30 rounded-lg border border-border flex gap-3">
                ...
                <p className="text-xs text-muted-foreground">
                  You will be notified when the book is ready for pickup. Reservations not collected within 3 days will
                  be cancelled.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleConfirmReserve} disabled={isProcessing} className="flex-1">
                {isProcessing ? (
                  <>
          ...
          <div className="p-6">
            ...
            <div className="space-y-4 mb-6">
              <div className="p-4 bg-gradient-to-br from-accent/10 to-primary/10 rounded-lg border border-accent/30">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-3xl font-bold text-accent">#{estimatedPosition}</p>
                    <p className="text-xs text-muted-foreground">{existingReservations.length} ahead of you</p>
                  </div>
                </div>
              </div>

              <div>
                <label className="text-sm font-semibold mb-3 block">Select Pickup Date</label>
                  ...
                  {availableDates.map((date, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Calendar
                          className={`w-5 h-5 ${selectedDate?.getTime() === date.getTime() ? "text-primary" : "text-muted-foreground"}`}
                        />
                      </div>
                    </button>
                  ))}
              ...
              <div>
                <label className="text-sm font-semibold mb-3 block">Borrow Duration</label>
                <div className="flex gap-2">
                  {[7, 14, 21].map((days) => (
                    <Button
                      key={days}
                      ...
                      className={`flex-1 ${borrowDuration !== days ? "bg-transparent" : ""}`}
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleProceed} disabled={!selectedDate} className="flex-1">
                Continue
              </Button>
            </div>
          </div>
