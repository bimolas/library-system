...
import { Star, BookOpen, Users, TrendingUp, Calendar, AlertCircle, ArrowLeft, Package, User } from "lucide-react"
...
export default function BookDetailPage() {
  ...
  const userAlreadyBorrowed = borrows.some(
    (b) => b.userId === currentUser.id && b.bookId === book.id && b.status === "active",
  )
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <Link href="/catalog">
          <Button variant="ghost" className="mb-6 gap-2 hover:translate-x-[-4px] transition-transform">
          ...
          <div className="lg:col-span-1 animate-fadeIn">
            <Card className="p-6 border-border sticky top-24">
              ...
              <div className="space-y-3 mb-6">
                ...
                {book.availableCopies === 0 && !userAlreadyBorrowed && (
                  <Button variant="outline" className="w-full bg-transparent" onClick={() => setShowReserveModal(true)}>
                    Join Waitlist
                  </Button>
                )}
              ...
              <div className="space-y-4 pt-6 border-t border-border">
                <div>
                  <p className="text-xs text-muted-foreground uppercase mb-2">Availability</p>
                  <div className="space-y-2">
                    <Badge
                      className={`${book.availableCopies > 0 ? "bg-success/10 text-success border-success/30" : "bg-destructive/10 text-destructive border-destructive/30"}`}
                    >
                      {book.availableCopies > 0 ? `${book.availableCopies} available now` : "All copies borrowed"}
                    </Badge>
                    <div className="flex items-center gap-2 text-sm">
                      <Package className="w-4 h-4 text-muted-foreground" />
                      <span className="text-muted-foreground">
                        {borrowedCopies} of {book.totalCopies} currently borrowed
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-xs text-muted-foreground uppercase">Total Copies</p>
                  <p className="mt-2 font-semibold text-lg">{book.totalCopies}</p>
                </div>

                <div>
                  <p className="text-xs text-muted-foreground uppercase">Demand Level</p>
                  <Badge className={`mt-2 ${demandColors[book.demandPressure]}`}>
                    <TrendingUp className="w-3 h-3 mr-1" />
                    {book.demandPressure.charAt(0).toUpperCase() + book.demandPressure.slice(1)} Demand
                  </Badge>
                </div>
              </div>
            </Card>
          ...
          <div className="lg:col-span-2 animate-fadeIn" style={{ animationDelay: "100ms" }}>
            <div className="mb-8">
              ...
              <div className="flex items-center gap-3 mb-4 p-3 bg-muted/30 rounded-lg border border-border">
                ...
                <div>
                  <p className="font-semibold text-lg text-primary">{book.author}</p>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-5 h-5 ${i < Math.floor(book.rating) ? "fill-amber-400 text-amber-400" : "text-border"}`}
                      />
                    ))}
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-border">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1">
                      <span className={`font-bold text-lg ${remainingBooks > 0 ? "text-success" : "text-destructive"}`}>
                        {remainingBooks}
                      </span>
                      <span className="text-muted-foreground"> of {book.totalCopies}</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            {book.availableCopies === 0 && (
              <Card className="p-6 border-border bg-amber-500/5 border-amber-500/30 mb-8 animate-fadeIn">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold mb-2">All Copies Currently Borrowed</h3>
                    <p className="text-sm text-muted-foreground mb-3">
                      This book is highly in demand. Reserve your copy now to get priority access when it becomes
                      available.
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Your score of <span className="font-semibold text-primary">{currentUser.score}</span> gives you
                      <span className="font-semibold text-accent"> {currentUser.level}</span> priority in the queue.
                    </p>
                  </div>
                </div>
              </Card>
