...
export default function UserDetailPage({ params }: PageProps) {
  ...
  const scoreHistory = [
    { month: "Jul", score: 8500 },
    { month: "Aug", score: 9200 },
    { month: "Sep", score: 9800 },
    { month: "Oct", score: 10500 },
    { month: "Nov", score: 11200 },
    { month: "Dec", score: 12450 },
  ]
    ...
    <div className="min-h-screen bg-background page-container">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Card className="p-6 border-border mb-6 animate-fadeIn">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="gap-2 bg-transparent hover-lift">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
          ...
          <Card className="p-4 border-border animate-slideIn" style={{ animationDelay: "30ms" }}>
            <div className="text-center">
              <p className="text-2xl font-bold">{user.totalBorrows}</p>
              <p className="text-xs text-muted-foreground">Total Borrows</p>
            </div>
          </Card>
          <Card className="p-4 border-border animate-slideIn" style={{ animationDelay: "60ms" }}>
            <div className="text-center">
              <p className="text-2xl font-bold text-emerald-600">{user.onTimeRate}%</p>
              <p className="text-xs text-muted-foreground">On-Time Rate</p>
            </div>
          </Card>
          <Card className="p-4 border-border animate-slideIn" style={{ animationDelay: "90ms" }}>
            <div className="text-center">
              <p className="text-2xl font-bold">{user.customBorrowDays}</p>
              <p className="text-xs text-muted-foreground">Borrow Days</p>
            </div>
          </Card>
          <Card className="p-4 border-border animate-slideIn" style={{ animationDelay: "120ms" }}>
            <div className="text-center">
              <p className="text-2xl font-bold text-red-600">{user.penalties}</p>
          ...
          <Card className="p-4 border-border animate-slideIn" style={{ animationDelay: "150ms" }}>
            <div className="text-center">
              <p className="text-2xl font-bold text-amber-600">{user.bonuses}</p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="text-center py-4 px-4 font-semibold">Borrow Date</th>
                      <th className="text-center py-4 px-4 font-semibold">Return Date</th>
                    ...
                    {borrowHistory.map((borrow, index) => (
                      ...
                      >
                        <td className="py-4 px-4">
                          <div>
                            <p className="font-semibold">{borrow.title}</p>
                            <p className="text-xs text-muted-foreground">{borrow.author}</p>
                          </div>
                        </td>
                        <td className="text-center py-4 px-4">{borrow.borrowDate.toLocaleDateString()}</td>
                        <td className="text-center py-4 px-4">{borrow.returnDate?.toLocaleDateString() || "-"}</td>
              ...
              <Card className="p-6 border-border">
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-red-500/5 border border-red-500/20 rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground">Atomic Habits - 3 days late</p>
                    <p className="text-xs text-red-600 mt-1">-50 points</p>
                  </div>
                  <div className="p-3 bg-red-500/5 border border-red-500/20 rounded-lg">
                    <div className="flex items-center justify-between mb-1">
                      <p className="font-medium text-sm">Missed Reservation</p>
                      <span className="text-xs text-muted-foreground">Aug 20, 2024</span>
                    </div>
                    <p className="text-xs text-muted-foreground">Did not pick up reserved book</p>
              ...
              <Card className="p-6 border-border">
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground">30 day reading streak</p>
                  ...
                  <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground">Read 5+ genres</p>
                    <p className="text-xs text-amber-600 mt-1">+75 points</p>
                  </div>
                  <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
                    <div className="flex items-center justify-between mb-1">
                      <p className="font-medium text-sm">Early Return</p>
                      <span className="text-xs text-muted-foreground">Nov 1, 2024</span>
                    </div>
                    <p className="text-xs text-muted-foreground">Returned 5+ days early</p>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                    ...
                    <Badge
                      className={
                        user.preferences.emailNotifications ? "bg-emerald-500/10 text-emerald-600" : "bg-muted"
                      }
                    >
                  ...
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                    ...
                    <Badge
                      className={user.preferences.borrowReminders ? "bg-emerald-500/10 text-emerald-600" : "bg-muted"}
                    >
                      {user.preferences.borrowReminders ? "Enabled" : "Disabled"}
                    </Badge>
                  </div>
                </div>
              </div>
